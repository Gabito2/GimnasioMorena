...
import {
  ...
  IonButton, IonNote, IonIcon,
} from '@ionic/angular';
import { ReactiveFormsModule, NonNullableFormBuilder, Validators } from '@angular/forms';
import { ToastController } from '@ionic/angular';
import { addIcons } from 'ionicons';
import { personOutline, callOutline, cashOutline, calendarOutline, saveOutline } from 'ionicons/icons';
...
export class Tab3Page {
  ...
  constructor() {
    addIcons({ personOutline, callOutline, cashOutline, calendarOutline, saveOutline });
  }
  ...
  async guardar(): Promise<void> {
    ...
    const toast = await this.toastCtrl.create({
      message: `${v.nombre} ${v.apellido} registrado. Cuota vence el ${this.formatear(venc.toISOString().slice(0, 10))}`,
      duration: 2600,
      color: 'success',
      position: 'top',
    });
